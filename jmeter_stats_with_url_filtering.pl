...
else
{
	...
	open(my_output_file, ">" . $fileCounter . "_" . $output_file);

	while (<CSV>) {
		next if ($. == 1);
	
		if ( $lineCounter == 900000 )
		{
			#print "**** Finished 900000 records";
			$fileCounter++;
			...
			# foreach my $key (sort {$err_timetaken{$b} <=> $err_timetaken{$a}} (keys(%err_timetaken))) {
				# print "* Responce times: $key\t=>\t" . $err_timetaken{$key} . "\t[" . sprintf("%.2f", ($err_timetaken{$key}/$lines_num*100)) . "%] \n";
			# }
		}
		...
		if ($csv->parse($_)) {
			...
			if ($columns[9] =~ /http:\/\/spuatdispatcher\/Comparison/)
			{
				print my_output_file localtime($columns[0]/1000) . "\t" . "TimeTaken=$columns[1]" . "\t" . "ResponseCode=$columns[3]" . "\t" . "RequestType=\"$columns[2]\"" . "\t" . "UserProfile=\"$columns[5]\"" . "\t" . "ConcurentUsers=$columns[10]" . "\t" .  "Size=$columns[8]" . "\t" . "Status=$columns[7]" . "\t" . "ResponseMessage=$columns[4]" . "\t" . "Url=\"$columns[9]\"" . "\n";
				
				$column	= $columns[1];				
				switch: {
					if ( $column <= 5 && $column > 0 ) { $err_timetaken{ '0 - 5(ms)' }++; }
					if ( $column <= 10 && $column > 5 ) { $err_timetaken{ '5 - 10(ms)' }++; }
					if ( $column <= 20 && $column > 10 ) { $err_timetaken{ '10 - 20(ms)' }++; }
					if ( $column <= 30 && $column > 20 ) { $err_timetaken{ '20 - 30(ms)' }++; }
					if ( $column <= 40 && $column > 30 ) { $err_timetaken{ '30 - 40(ms)' }++; }
					if ( $column <= 50 && $column > 40 ) { $err_timetaken{ '40 - 50(ms)' }++; }
					...
					if ( $column > 10000 ) { $err_timetaken{ '10+(sec)' }++; }
				}
